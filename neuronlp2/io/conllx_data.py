...
def read_data_to_variable(source_path,
                          word_alphabet,
                          char_alphabet,
                          pos_alphabet,
                          type_alphabet,
                          ...
                          lang_id="",
                          len_thresh=100000):
    data, max_char_length = read_data(source_path,
                                      word_alphabet,
                                      char_alphabet,
                                      pos_alphabet,
                                      type_alphabet,
    ...
    for bucket_id in range(len(_buckets)):
        ...
        for i, inst in enumerate(data[bucket_id]):
            wids, cid_seqs, pids, hids, tids, berts = inst
            inst_size = len(wids)
            lengths[i] = inst_size
            # word ids
            wid_inputs[i, :inst_size] = wids
...
def get_batch_variable(data, batch_size, unk_replace=0.):
    ...
    words, chars, pos, heads, types, masks, single, lengths, bert = data_variable[bucket_id]
    ...
    if unk_replace:
        ...
        noise = masks.new(batch_size, bucket_length).bernoulli_(unk_replace).long()
...
def iterate_batch_variable(data, batch_size, unk_replace=0., shuffle=False):
    ...
    for bucket_id in bucket_indices:
        ...
        if unk_replace:
            ...
            noise = masks.new(bucket_size, bucket_length).bernoulli_(unk_replace).long()
        ...
        for start_idx in range(0, bucket_size, batch_size):
            ...
            if bert_inputs_ids is not None:
                yield words[excerpt], chars[excerpt], pos[excerpt], heads[excerpt], types[excerpt], \
                      masks[excerpt], lengths[excerpt], [bert_inputs_ids[excerpt], bert_input_mask[excerpt],
                                                         bert_out_positions[excerpt]]
            else:
                yield words[excerpt], chars[excerpt], pos[excerpt], heads[excerpt], types[excerpt], \
                      masks[excerpt], lengths[excerpt], [None, None, None]
