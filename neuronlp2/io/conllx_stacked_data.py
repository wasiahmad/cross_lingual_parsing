...
def read_stacked_data(source_path,
    ...
    counter = 0
    counter_added = 0
    reader = CoNLLXReader(source_path,
                          word_alphabet,
                          char_alphabet,
                          pos_alphabet,
                          type_alphabet,
                ...
                if inst_size < bucket_size:
                    ...
                    data[bucket_id].append(
                        [sent.word_ids, sent.char_id_seqs, inst.pos_ids, inst.heads, inst.type_ids, stacked_heads,
                         children, siblings, stacked_types, skip_connect, sent.bert_tokens])
                    max_len = max([len(char_seq) for char_seq in sent.char_seqs])
                    if max_char_length[bucket_id] < max_len:
                        max_char_length[bucket_id] = max_len
                    break
            counter_added += 1
            if inst_size > max_instance_length:
                max_instance_length = inst_size

        inst = reader.getNext(normalize_digits=normalize_digits, symbolic_root=True, symbolic_end=False)
    reader.close()
    print("Total number of data: %d, used: %d" % (counter, counter_added))
    print("Maximum instance length: %d" % max_instance_length)
    return data, max_char_length


def read_stacked_data_to_variable(source_path,
                                  word_alphabet,
                                  char_alphabet,
                                  pos_alphabet,
                                  type_alphabet,
                                  ...
                                  len_thresh=100000,
                                  use_bert=False):
    data, max_char_length = read_stacked_data(source_path,
                                              word_alphabet,
                                              char_alphabet,
                                              pos_alphabet,
                                              type_alphabet,
    ...
    for bucket_id in range(len(_buckets)):
        ...
        bert_inputs_ids, bert_input_mask, bert_out_positions = None, None, None
        bert_max_length = 0
        if use_bert:
            bert_max_length = max([len(inst[-1]['input_ids']) for inst in data[bucket_id]])
            bert_inputs_ids = np.zeros([bucket_size, bert_max_length], dtype=np.int64)
            bert_input_mask = np.zeros([bucket_size, bert_max_length], dtype=np.int64)
            bert_out_positions = np.empty([bucket_size, bucket_length], dtype=np.int64)

        for i, inst in enumerate(data[bucket_id]):
            wids, cid_seqs, pids, hids, tids, stack_hids, chids, ssids, stack_tids, skip_ids, berts = inst
            inst_size = len(wids)
            ...
            if use_bert:
                ...
                required_pad = bucket_length - len(berts['out_positions'])
                if required_pad > 0:
        ...
        if use_bert:
            bert_inputs_ids = torch.from_numpy(bert_inputs_ids)
            bert_input_mask = torch.from_numpy(bert_input_mask)
            bert_out_positions = torch.from_numpy(bert_out_positions)
        ...
        if use_gpu:
            ...
            if use_bert:
                bert_inputs_ids = bert_inputs_ids.cuda()
                bert_input_mask = bert_input_mask.cuda()
                bert_out_positions = bert_out_positions.cuda()

        data_variable.append(((words, chars, pos, heads, types, masks_e, single, lengths_e,
                               [bert_inputs_ids, bert_input_mask, bert_out_positions]),
...
def get_batch_stacked_variable(data, batch_size, unk_replace=0.):
    ...
    words, chars, pos, heads, types, masks_e, single, lengths_e, bert = data_encoder
    bert_inputs_ids, bert_input_mask, bert_out_positions = bert
    ...
    words = words[index]
    if unk_replace:
        ones = single.new(batch_size, bucket_length).fill_(1)
        noise = masks_e.new(batch_size, bucket_length).bernoulli_(unk_replace).long()
        words = words * (ones - single[index] * noise)

    input_ids, input_mask, out_positions = None, None, None
    if bert_inputs_ids is not None:
        input_ids = bert_inputs_ids[index]
    if bert_input_mask is not None:
        input_mask = bert_input_mask[index]
    if bert_out_positions is not None:
        out_positions = bert_out_positions[index]

    return (words, chars[index], pos[index], heads[index], types[index], masks_e[index], lengths_e[index],
            [input_ids, input_mask, out_positions]), \
...
def iterate_batch_stacked_variable(data, batch_size, unk_replace=0., shuffle=False):
    ...
    for bucket_id in bucket_indices:
        ...
        if bucket_size == 0:
            continue
        data_encoder, data_decoder = data_variable[bucket_id]
        words, chars, pos, heads, types, masks_e, single, lengths_e, berts = data_encoder
        bert_inputs_ids, bert_input_mask, bert_out_positions = berts
        stacked_heads, children, siblings, stacked_types, skip_connect, masks_d, lengths_d = data_decoder
        if unk_replace:
            ones = single.new(bucket_size, bucket_length).fill_(1)
            noise = masks_e.new(bucket_size, bucket_length).bernoulli_(unk_replace).long()
        ...
        for start_idx in range(0, bucket_size, batch_size):
            ...
            if bert_inputs_ids is not None:
                yield (words[excerpt], chars[excerpt], pos[excerpt], heads[excerpt], types[excerpt], masks_e[excerpt],
                       lengths_e[excerpt], [bert_inputs_ids[excerpt], bert_input_mask[excerpt],
                                            bert_out_positions[excerpt]]), \
