...
class AugBiAAttention(nn.Module):
    def __init__(self, input_size_encoder, input_size_decoder, num_labels, num_features, dim_feature, drop_f_embed,
                 biaffine=True):
        ...
        if self.use_features:  # only meaningful if >1 features
            self.E = nn.Embedding(num_features, dim_feature)
    ...
    def forward(self, input_d, input_e, input_features, mask_d=None, mask_e=None):
        ...
        if self.use_features:
            features_embed = self.E_drop(self.E(input_features))  # [batch, len-d, len-e, fdim]
