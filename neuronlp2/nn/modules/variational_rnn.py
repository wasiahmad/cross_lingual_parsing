...
class VarRNNCell(VarRNNCellBase):
    ...
    def reset_noise(self, batch_size):
        if self.training:
            ...
            if self.p_hidden:
                noise = self.weight_hh.new(batch_size, self.hidden_size)
...
class VarLSTMCell(VarRNNCellBase):
    ...
    def reset_noise(self, batch_size):
        if self.training:
            ...
            if self.p_hidden:
                noise = self.weight_hh.new(4, batch_size, self.hidden_size)
                self.noise_hidden = noise.bernoulli_(1.0 - self.p_hidden) / (1.0 - self.p_hidden)
            else:
                self.noise_hidden = None
        else:
...
class VarGRUCell(VarRNNCellBase):
    ...
    def reset_noise(self, batch_size):
        if self.training:
            ...
            if self.p_hidden:
                noise = self.weight_hh.new(3, batch_size, self.hidden_size)
                self.noise_hidden = noise.bernoulli_(1.0 - self.p_hidden) / (1.0 - self.p_hidden)
            else:
                self.noise_hidden = None
        else:
