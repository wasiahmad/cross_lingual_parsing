...
class SkipConnectRNNBase(nn.Module):
    ...
    def forward(self, input, skip_connect, mask=None, hx=None):
        ...
        if hx is None:
            ...
            hx = input.new(self.num_layers * num_directions, batch_size, self.hidden_size).zero_()
            if self.lstm:
                hx = (hx, hx)
        ...
        output, hidden = func(input, skip_connect, self.all_cells, hx,
    ...
    def step(self, input, hx=None, hs=None, mask=None):
        ...
        if hx is None:
            hx = input.new(self.num_layers, batch_size, self.hidden_size).zero_()
            if self.lstm:
                hx = (hx, hx)
        if hs is None:
            hs = input.new(self.num_layers, batch_size, self.hidden_size).zero_()
