...
def SkipConnectRNNReLUCell(input, hidden, hidden_skip, w_ih, w_hh, b_ih=None, b_hh=None, noise_in=None,
                           noise_hidden=None, noise_skip=None):
...
def SkipConnectRNNTanhCell(input, hidden, hidden_skip, w_ih, w_hh, b_ih=None, b_hh=None, noise_in=None,
                           noise_hidden=None):
    if noise_in is not None:
        input = input * noise_in
...
def SkipConnectFastLSTMCell(input, hidden, hidden_skip, w_ih, w_hh, b_ih=None, b_hh=None, noise_in=None,
                            noise_hidden=None):
    if noise_in is not None:
        input = input * noise_in
...
def SkipConnectFastGRUCell(input, hidden, hidden_skip, w_ih, w_hh, b_ih=None, b_hh=None, noise_in=None,
                           noise_hidden=None):
    if noise_in is not None:
        input = input * noise_in
...
def SkipConnectRecurrent(reverse=False):
    def forward(input, skip_connect, hidden, cell, mask):
        ...
        output = input.new(input.size(0) + 1, *h0.size()).zero_() + h0
        steps = range(input.size(0) - 1, -1, -1) if reverse else range(input.size(0))
        # create batch index
        batch_index = torch.arange(0, h0.size(0)).type_as(skip_connect)
        for i in steps:
            if mask is None or mask[i].min() > 0.5:
...
def SkipConnectStep():
    def forward(input, hidden, hidden_skip, cell, mask):
        if mask is None or mask.min() > 0.5:
