...
def biaffine(model_path, model_name, test_path, punct_set, use_gpu, logger, args):
    ...
    def _read_one(path, is_train):
        ...
        one_data = conllx_data.read_data_to_variable(path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet,
                                                     use_gpu=False, volatile=(not is_train), symbolic_root=True,
                                                     lang_id=lang_id, use_bert=kwargs['use_bert'])
        return one_data
...
def stackptr(model_path, model_name, test_path, punct_set, use_gpu, logger, args):
    ...
    with torch.no_grad():
        for batch in conllx_stacked_data.iterate_batch_stacked_variable(data_test, 1):
            ...
            stacked_heads, children, _, stacked_types, _, mask_d, _ = input_decoder
            ...
            heads_pred, types_pred, children_pred, stacked_types_pred = \
                network.decode(word, char, pos, input_bert=bert_inputs, mask=masks, length=lengths, beam=beam,
                               ordered=ordered, leading_symbolic=conllx_stacked_data.NUM_SYMBOLIC_TAGS)
    ...
    def analyze():
        ...
        for gold, pred in zip(gold_iter, test_iter):
            ...
            ucorr_stack = (ucorr_stack * gold_mask_d).sum()
            lcorr_stack = (lcorr_stack * gold_mask_d).sum()
            num_stack = gold_mask_d.sum()
            ...
            if lcorr_stack < num_stack:
                ...
                if display_inst:
                    ...
                    word = word[0].cpu().numpy()
                    pos = pos[0].cpu().numpy()
                    head_gold = gold_heads[0].cpu().numpy()
                    type_gold = gold_types[0].cpu().numpy()
                    head_pred = pred_heads[0].cpu().numpy()
                    type_pred = pred_types[0].cpu().numpy()
                    ...
                    gold_display[0] = gold_stacked_types[0].cpu().numpy()[:length_dec]
                    gold_display[1] = gold_children[0].cpu().numpy()[:length_dec]
                    gold_display[2] = gold_stacked_heads[0].cpu().numpy()[:length_dec]
                    print(gold_display)
                    print('--------------------------------------------------------')
                    pred_display = np.empty([3, pred_lengths_d[0]])[:length_dec]
                    pred_display[0] = pred_stacked_types[0].cpu().numpy()[:length_dec]
                    pred_display[1] = pred_children[0].cpu().numpy()[:length_dec]
                    pred_display[2] = pred_stacked_heads[0].cpu().numpy()[:length_dec]
                    print(pred_display)
                    print('========================================================')
                    input()
...
def calc_loss(network, word, char, pos, heads, stacked_heads, children, sibling, stacked_types, skip_connect, mask_e,
    ...
    err_arc_leaf = loss_arc_leaf.item() * num_leaf
    err_arc_non_leaf = loss_arc_non_leaf.item() * num_non_leaf

    err_type_leaf = loss_type_leaf.item() * num_leaf
    err_type_non_leaf = loss_type_non_leaf.item() * num_non_leaf

    err_cov = loss_cov.item() * (num_leaf + num_non_leaf)
...
def display(word, pos, head_gold, type_gold, head_pred, type_pred, length, word_alphabet, pos_alphabet, type_alphabet):
    for j in range(0, length):
        w = word_alphabet.get_instance(word[j])
        p = pos_alphabet.get_instance(pos[j])
        t_g = type_alphabet.get_instance(type_gold[j])
        h_g = head_gold[j]
        t_p = type_alphabet.get_instance(type_pred[j])
        h_p = head_pred[j]
